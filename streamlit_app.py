...
def process_text_to_csv(text_content, legal_name, company_number, statement_number):
    """Process text content to generate a CSV with vertical and horizontal headers."""
    ...
    # Set horizontal headers at row 1 for columns B to E
    horizontal_headers = ["Shareholding #", "Amount of Shares", "Type of Shares", "Shareholder Name"]
    if len(csv_data) < 1:
        csv_data.append([""] + horizontal_headers)  # Ensure row 1 has these headers
    else:
        csv_data[0].extend(horizontal_headers)  # Place them in the first row of columns B:E

    # Extract company and shareholder details
    statement_date = ""
    shareholder_data = []
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith("Confirmation Statement date:"):
            ...
            csv_data[6][0] = statement_date  # Update statement date in the vertical header
        ...
        if line.startswith("Shareholding"):
            ...
            # Add the shareholder data
            shareholder_data.append(
                [shareholding_number, amount_of_shares, type_of_shares, shareholder_name or "PENDING"]
            )

    # Append shareholder data starting from row 2 in columns B:E
    for idx, row in enumerate(shareholder_data, start=1):
        while len(csv_data) <= idx:
            csv_data.append([""])  # Ensure alignment
        csv_data[idx].extend([""] + row)  # Align rows starting from column B
